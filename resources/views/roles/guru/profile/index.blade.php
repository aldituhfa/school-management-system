        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body p-0 position-relative">
                ...
                <form id="formProfile" action="{{ route('superadmin.profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="text-center position-relative" style="margin-top: -60px;">
                        <div class="position-relative d-inline-block">
                            ...
                            @else
                                {{-- Jika belum ada foto profil, tampilkan inisial nama --}}
                                @php
                                    $nama = $user->name;
                                    $inisial = collect(explode(' ', $nama))->map(fn($n) => strtoupper(substr($n, 0, 1)))->join('');
                                @endphp
                                <div id="profilePreview"
                                    class="rounded-circle border border-2 border-white shadow bg-primary text-white d-flex align-items-center justify-content-center"
                                    style="width:115px; height:115px; font-size:36px; font-weight:bold;">
                                    {{ $inisial }}
                                </div>
                            @endif
                        ...
                        <div class="card-body pt-0">
                            ...
                            <div class="row align-items-end">
                                <div class="col-md-6">
                                    ...
                                    <small class="text-muted" style="font-size: 13px;">Hubungi administrator untuk mengganti password.</small>
                                </div>
                ...
                <div class="card shadow-sm border-0 rounded-3 mt-3">
                    <div class="card-header bg-white border-bottom-0 pb-0">
                        <h5 class="card-title text-primary mb-2" style="font-size: 17px;">
                            <i class="ti ti-book me-1"></i> Mata Pelajaran yang Diajarkan
                        </h5>
                    </div>

                    <div class="card-body pt-0">
                        @if($mataPelajaran && $mataPelajaran->count() > 0)
                            <ul class="list-group list-group-flush">
                                @foreach($mataPelajaran as $mapel)
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <span>
                                            <i class="ti ti-book-2 text-primary me-2"></i> {{ $mapel->nama_mata_pelajaran }}
                                        </span>
                        ...
                        @else
                            <p class="text-muted mb-0" style="font-size: 15px;">
                                Guru ini belum memiliki mata pelajaran terdaftar.
                            </p>
                        @endif
                    </div>
                </div>
        ...
        if (file) {
            ...
            reader.onload = function(evt) {
                ...
                if (preview.tagName.toLowerCase() !== 'img') {
                    ...
                    // Ganti elemen div lama dengan img baru
                    preview.parentNode.replaceChild(img, preview);
                    preview = img; // ⬅️ UPDATE variabel preview ke elemen baru
                }
