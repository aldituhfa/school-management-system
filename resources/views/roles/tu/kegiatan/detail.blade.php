        ...
        <div class="card p-3 shadow-sm">
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    @php
                        $p = $pembayarans->get($s->id); // pembayaran untuk siswa ini (boleh null)
                    @endphp
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $s->nama_siswa }}</td>
                        ...
                        <td>
                            @if(!$p || $p->status == 'belum')
                                <form method="POST" action="{{ route('tu.kegiatan.bayar', [$kegiatan->id, $s->id]) }}" onsubmit="return confirm('Konfirmasi bayar untuk {{ $s->nama_siswa }}?');">
                                    @csrf
                                    <button class="btn btn-sm btn-dark" type="submit">Bayar</button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('tu.kegiatan.cancel', [$kegiatan->id, $s->id]) }}" onsubmit="return confirm('Batalkan pembayaran untuk {{ $s->nama_siswa }}?');">
                                    @csrf
                    ...
                    @if($siswa->isEmpty())
                    <tr>
                        <td colspan="7" class="text-center">Belum ada siswa untuk kelas yang dipilih.</td>
                    </tr>
                    @endif
                </tbody>
            </table>
